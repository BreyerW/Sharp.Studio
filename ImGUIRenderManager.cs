...
namespace ImGuiNET
{
    ...
    /// </summary>
    public class ImGuiRenderManager : IDisposable
    {
        ...
        private readonly Window _mainViewportWindow;
        ...
        /// </summary>
        public unsafe ImGuiRenderManager(GraphicsDevice gd, Sdl2Window window, OutputDescription outputDescription, int width, int height)
        {
            _gd = gd;
            _window = window;
            _windowWidth = width;
            ...
            _mainViewportWindow = new Window(gd, mainViewport, _window);
        ...
        {
            Window window = new Window(_gd, vp);
        }
        ...
        {
            Window window = (Window)GCHandle.FromIntPtr(vp.PlatformUserData).Target;
            Sdl2Native.SDL_ShowWindow(window.NativeWindow.SdlWindowHandle);
        }

        private unsafe void GetWindowPos(ImGuiViewportPtr vp, Vector2* outPos)
        {
            Window window = (Window)GCHandle.FromIntPtr(vp.PlatformUserData).Target;
            *outPos = new Vector2(window.NativeWindow.Bounds.X, window.NativeWindow.Bounds.Y);
        }

        private void SetWindowPos(ImGuiViewportPtr vp, Vector2 pos)
        {
            Window window = (Window)GCHandle.FromIntPtr(vp.PlatformUserData).Target;
            window.NativeWindow.X = (int)pos.X;
            window.NativeWindow.Y = (int)pos.Y;
        }

        private void SetWindowSize(ImGuiViewportPtr vp, Vector2 size)
        {
            Window window = (Window)GCHandle.FromIntPtr(vp.PlatformUserData).Target;
            Sdl2Native.SDL_SetWindowSize(window.NativeWindow.SdlWindowHandle, (int)size.X, (int)size.Y);
        }

        private unsafe void GetWindowSize(ImGuiViewportPtr vp, Vector2* outSize)
        {
            Window window = (Window)GCHandle.FromIntPtr(vp.PlatformUserData).Target;
            Rectangle bounds = window.NativeWindow.Bounds;
        ...
        private void SetWindowFocus(ImGuiViewportPtr vp)
        {
            ...
            Window window = (Window)GCHandle.FromIntPtr(vp.PlatformUserData).Target;
            p_sdl_RaiseWindow(window.NativeWindow.SdlWindowHandle);
        }

        private byte GetWindowFocus(ImGuiViewportPtr vp)
        {
            Window window = (Window)GCHandle.FromIntPtr(vp.PlatformUserData).Target;
            SDL_WindowFlags flags = Sdl2Native.SDL_GetWindowFlags(window.NativeWindow.SdlWindowHandle);
        ...
        private unsafe void SetWindowTitle(ImGuiViewportPtr vp, IntPtr title)
        {
            ...
            window.NativeWindow.Title = System.Text.Encoding.ASCII.GetString(titlePtr, count);
        }
        ...
        {
            //Assembly assembly = typeof(ImGuiRenderManager).Assembly;
            //assembly.GetManifestResourceStream(resourceName)
            using (Stream s = new FileStream(@"C:\Users\damia\source\repos\Sharp.Studio\Shaders\HLSL\" + resourceName, FileMode.Open))
            {
        ...
        private void UpdateImGuiInput(InputSnapshot snapshot)
        {
            ...
            for (int i = 1; i < viewports.Size; i++)
            {
                ...
                Window window = ((Window)GCHandle.FromIntPtr(v.PlatformUserData).Target);
                window.Update();
            }
        }
